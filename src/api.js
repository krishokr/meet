...
export const getAccessToken = async () => {
    ...
    if (!accessToken || tokenCheck.error) {
        ...
        if (!code) {
            const results = await axios.get("https://aiaelv39hf.execute-api.us-west-1.amazonaws.com/dev/api/get-auth-url");
...
export const checkToken = async (accessToken) => {
  ...
  const getToken = async (code) => {
    ...
    const { access_token } = await fetch(
      'https://aiaelv39hf.execute-api.us-west-1.amazonaws.com/dev/api/token' + '/' + encodeCode
    )
      .then((res) => {
        return res.json();
      })
